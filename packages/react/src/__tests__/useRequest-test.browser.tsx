...
describe('useRequest', () => {
    it('auto-dispatches on mount and transitions loading → success', async () => {
    ...
    it('refresh() after an error returns to loading while preserving the stale error', async () => {
        const boom = new Error('boom');
        const req = makeFakeRequest<string>();
        const { result } = renderHook(() => useRequest(req.source));
    ...
    it('refresh() re-dispatches and returns to loading while preserving stale data', async () => {
        const req = makeFakeRequest<string>();
        const { result } = renderHook(() => useRequest(req.source));
    ...
    it('refresh({ signal }) overrides the perRequestSignal factory for that attempt', async () => {
        const req = makeFakeRequest<string>();
        const perRequestSignal = jest.fn(() => new AbortController().signal);
        const { result } = renderHook(() => useRequest(req.source, { perRequestSignal }));
        ...
        act(() => result.current.refresh({ signal: overrideCtrl.signal }));
        expect(perRequestSignal).toHaveBeenCalledTimes(1);
        expect(req.fn).toHaveBeenCalledTimes(2);
    ...
    it('refresh() without a signal arg falls back to the perRequestSignal factory', () => {
        const req = makeFakeRequest<string>();
        const perRequestSignal = jest.fn(() => new AbortController().signal);
        const { result } = renderHook(() => useRequest(req.source, { perRequestSignal }));
    ...
    it('refresh({ signal: undefined }) opts out of the perRequestSignal factory for that attempt', () => {
        const req = makeFakeRequest<string>();
        const perRequestSignal = jest.fn(() => new AbortController().signal);
        const { result } = renderHook(() => useRequest(req.source, { perRequestSignal }));
        ...
        act(() => result.current.refresh({ signal: undefined }));
        expect(perRequestSignal).toHaveBeenCalledTimes(1);
        expect(req.fn).toHaveBeenCalledTimes(2);
    });
    ...
    describe('function shape', () => {
        ...
        it('combines a per-request signal into the signal passed to the function', () => {
            ...
            const ctrl = new AbortController();
            renderHook(() => useRequest(fn, { perRequestSignal: () => ctrl.signal }));
        ...
        it('rebuilds the store and fires a fresh request when one function source replaces another', () => {
            ...
            const { rerender } = renderHook(({ fn }: { fn: (signal: AbortSignal) => Promise<string> }) => useRequest(fn), {
                initialProps: { fn: fnA },
            });
